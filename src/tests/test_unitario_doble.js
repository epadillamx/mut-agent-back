...
describe('Chat Controller Tests', () => {
    ...
    test('conversación completa flujo 1', async () => {
        ...
        // Mensaje 1: Saludo
        let message = 'me llamo eduardo padilla, epadilla@com.com, hay una incendio  en el paso se esta tirando el agua me urge , estoy localizado en el local Carlos';
        ...
        message = 'es Local';
        const response8 = await processMessage(number, message);
        await new Promise(resolve => setTimeout(resolve, 100));

        assert.ok(typeof response8 === 'string', 'Debe recibir respuesta de confirmación');
        conversationLog.push({ user: message, agente: response8 });

        logger.warn("Test - Conversación:", JSON.stringify({
            user: message,
            agente: "ok"
        }, null, 2));
        logger.warn(response8)
        ...
        assert.strictEqual(conversationLog.length, 3, 'Debe tener exactamente 2 intercambios');
